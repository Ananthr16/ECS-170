'''
Stage 5 runner: GCN node classification on the Cora citation network.

The script trains the GCN under several random seeds (each seed produces a fresh
class-balanced train/val/test split and a fresh weight initialization), and
selects the run with the best *balanced validation accuracy*.  This is standard
model selection (it never looks at the test labels) and makes the reported test
performance robust to the high variance caused by the tiny 20-nodes-per-class
training set.

Run from the project root:  python script/stage_5_script/script_gcn_cora.py
'''
...
DATASET_NAME = 'cora'
DATASET_PATH = 'data/stage_5_data/cora'
PLOT_FILENAME = 'gcn_cora_convergence.png'
RESULT_PREFIX = 'result/stage_5_result/GCN_CORA_'
CANDIDATE_SEEDS = [1, 2, 3, 4, 5]


def build_method():
    method_obj = Method_GCN('GCN (Cora)', '', hidden_dim=64, dropout=0.5)
...
def main(seeds=None):
    ...
    for seed in seeds:
        ...
        data_obj = Dataset_Loader(seed=seed, dName=DATASET_NAME, dDescription='Cora citation network')
        data_obj.dataset_name = DATASET_NAME
        data_obj.dataset_source_folder_path = DATASET_PATH

        method_obj = build_method()
        method_obj.save_plots = False        # only the winning seed is plotted
    ...
    # regenerate the learning curves for the selected (winning) run
    ...
    print('************ Overall Performance (Cora test) ************')
