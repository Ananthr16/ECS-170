...
if __name__ == '__main__':
    ...
    data_obj.max_vocab_size = 1000 if smoke else 25000
    ...
    method_obj = Method_RNN_Classifier('lstm text classifier', '', cell_type='lstm')
    method_obj.plot_filename = 'lstm_classification_convergence.png'
    method_obj.max_epoch = 1 if smoke else 10
    method_obj.batch_size = 32 if smoke else 128
    method_obj.embedding_dim = 32 if smoke else 128
    method_obj.hidden_size = 32 if smoke else 160
    method_obj.dropout = 0.2 if smoke else 0.35
    ...
    print('************ Start Stage 4 LSTM Classification ************')
    setting_obj.prepare(data_obj, method_obj, result_obj, evaluate_obj)
    setting_obj.print_setup_summary()

    scores, _ = setting_obj.load_run_save_evaluate()

    print('************ Overall Performance (LSTM classification test) ************')
