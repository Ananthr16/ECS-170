...
if __name__ == '__main__':
    ...
    method_obj = Method_RNN_Generator('rnn text generator', '', cell_type='rnn')
    method_obj.plot_filename = 'rnn_generation_convergence.png'
    method_obj.max_epoch = 2 if smoke else 50
    ...
    print('************ Start Stage 4 RNN Generation ************')
    setting_obj.prepare(data_obj, method_obj, result_obj, evaluate_obj)
    setting_obj.print_setup_summary()

    scores, _ = setting_obj.load_run_save_evaluate()

    print('************ Overall Performance (RNN generation validation) ************')
